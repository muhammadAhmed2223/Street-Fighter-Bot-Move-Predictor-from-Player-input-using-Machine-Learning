...
class Bot:
    ...
    def fight(self,current_game_state,player):
        ...
        if player=="1":
            ...
            columns = [ 'health','x_coord', 'y_coord',
                       'is_jumping','is_crouching','is_player_in_move','move_id',
                        'Player2health','Player2x_coord','Player2y_coord',
                        'Player2is_jumping',
                        'Player2is_crouching','Player2is_player_in_move','Player2move_id','player2_buttons up',
            ...
            columns = [ 'health','x_coord', 'y_coord',
                       'is_jumping','is_crouching','is_player_in_move','move_id',
                        ...
                        'Player2health','Player2x_coord','Player2y_coord',
                        'Player2is_jumping',
                        'Player2is_crouching','Player2is_player_in_move','Player2move_id'
                       ]

            # create a DataFrame
            df = pd.DataFrame(data.reshape(1, -1), columns=columns)
            pred = self.model2.predict(df)
            pred = np.round(pred)
            pred = np.absolute(pred)
            if( self.exe_code!=0 ):
